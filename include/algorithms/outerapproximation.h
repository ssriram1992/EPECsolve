...
public:
	...
	public:
		...
		inline std::vector<unsigned int> getIdComps () const { return this->IdComps; } ///< Getter method for idComp
		...
		inline std::vector<bool> getEncoding () const { return this->Encoding; } ///< Getter method for the encoding.

		inline std::vector<bool>
		getAllowedBranchings () const { return this->AllowedBranchings; } ///< Getter method for the allowed branchings

		inline Node *getParent () const { return this->Parent; }///< Getter method for the parent node

	private:
		std::vector<unsigned int> IdComps;     ///< Contains the branching decisions taken at the node
		std::vector<bool> Encoding; ///< An encoding of bool. True if the complementarity condition is included in the current node outer approximation, false otherwise.
		std::vector<bool> AllowedBranchings; ///< A vector where true means that the corresponding complementarity is a candidate for banching at the current node
...
private:
	...
	unsigned int EncodingSize = 0;///< The size of the encoding, namely the number of complementarity equations
...
///@brief This class is responsible for the outer approximation Algorithm
	...
	private:
		...
		std::vector<int> getNextBranchLocation (const unsigned int player, const OuterTree::Node *node);
		int getFirstBranchLocation (const unsigned int player, const OuterTree::Node *node);

	public:
		friend class EPEC;

		OuterApproximation (GRBEnv *env, Game::EPEC *EpecObj) : env{env}, EPECObject{EpecObj} {
			/*
