...
namespace Models {
    ...
    enum class TaxType {
        StandardTax, SingleTax, CarbonTax
    };

/// @brief Stores the parameters of the follower in a country model
    struct FollPar {

        std::vector<double> costs_quad = {}; ///< Quadratic coefficient of i-th follower's cost. Size of this
        ///< std::vector should be equal to n_followers
        std::vector<double> costs_lin = {}; ///< Linear  coefficient of i-th follower's cost. Size of this
        ///< std::vector should be equal to n_followers
        std::vector<double> capacities = {}; ///< Production capacity of each follower. Size of this std::vector
        ///< should be equal to n_followers
        std::vector<double> emission_costs = {}; ///< Emission costs for unit quantity of the fuel. Emission costs
        ...
        FollPar (std::vector<double> costs_quad_ = {}, std::vector<double> costs_lin_ = {},
                 ...
                 std::vector<double> tax_caps_ = {}, std::vector<std::string> names_ = {}) : costs_quad{costs_quad_},
                                                                                             costs_lin{costs_lin_},
...
/// @brief Stores the parameters of the leader in a country model
    struct LeadPar {
        ...
        Models::TaxType tax_type = Models::TaxType::StandardTax; ///< 0 For standard, 1 for constant tax, 2
        ///< for carbon tax
        ...
        LeadPar (double imp_lim = -1, double exp_lim = -1, double price_limit = -1, bool tax_revenue = false,
                 unsigned int tax_type_ = 0) : import_limit{imp_lim}, export_limit{exp_lim}, price_limit{price_limit},
...
/// @brief Stores the parameters of a country model
    struct LeadAllPar {
        ...
        LeadAllPar (unsigned int n_foll, std::string name, Models::FollPar FP = {}, Models::DemPar DP = {},
                    Models::LeadPar LP = {}) : n_followers{n_foll}, name{std::move (name)}, FollowerParam{FP},
...
/// @brief Stores a single Instance
    struct EPECInstance {
        ...
        EPECInstance (std::vector<Models::LeadAllPar> Countries_, arma::sp_mat Transp_) : Countries{Countries_},
                                                                                          TransportationCosts{
                                                                                              Transp_} {}
    ...
    enum class LeaderVars {
        FollowerStart, NetImport, NetExport, CountryImport, Caps, Tax, TaxQuad, DualVar, ConvHullDummy, End
    };
    ...
    private:
        ...
        std::vector<std::shared_ptr<Game::QP_Param>> MC_QP = {}; ///< The QP corresponding to the market clearing condition of each
        ///< player
        ...
        bool dataCheck (bool chkAllLeadPars = true, bool chkcountriesLL = true, bool chkMC_QP = true,
                        ...
                        bool chkLocations = true, bool chkLeaderLocations = true, bool chkLeadObjec = true) const;
        ...
        void make_LL_QP (const LeadAllPar &Params, const unsigned int follower, Game::QP_Param *Foll,
                         const LeadLocs &Loc) noexcept;

        /// Makes the leader constraint matrix and RHS
        void make_LL_LeadCons (arma::sp_mat &LeadCons, arma::vec &LeadRHS, const LeadAllPar &Param,
                               const Models::LeadLocs &Loc = {}, const unsigned int import_lim_cons = 1,
                               const unsigned int export_lim_cons = 1, const unsigned int price_lim_cons = 1,
        ...
        void WriteCountry (const unsigned int i, const std::string filename, const arma::vec x,
                           const bool append = true) const;

        void
        WriteFollower (const unsigned int i, const unsigned int j, const std::string filename, const arma::vec x) const;
    ...
    public:                        // Attributes
        ...
        unsigned int
        getPosition (const std::string &countryCount, const LeaderVars var = LeaderVars::FollowerStart) const;
