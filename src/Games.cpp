...
void Game::NashGame::save(const string &filename, bool erase) const {
  ...
  Utils::appendSave(this->NumPlayers, filename, string("NashGame::NumPlayers"),
                    false);
  ...
  Utils::appendSave(this->LeaderConstraintsRHS, filename,
                    string("NashGame::LeaderConstraintsRHS"), false);
  Utils::appendSave(this->numLeaderVar, filename,
                    string("NashGame::numLeaderVar"), false);
  ...
  if (this->LeaderConstraints.n_rows) {
    ...
    case -1:
      this->LeaderConstraints =
          resizePatch(this->LeaderConstraints, nnR, nnC + par);
      break;
    case 0:
...
void Game::NashGame::write(const string &filename, bool append,
                           bool KKT) const {
  ofstream file;
 ...
 */
{
  ...
  try {
    ...
    this->LeaderObjectiveConvexHull =
        vector<shared_ptr<Game::QP_Objective>>(NumPlayers);
...
void EPEC::getXMinusI(const arma::vec &x, const unsigned int &i,
 ...
 */
{
  ...
  if (!this->NashEquilibrium)
    return false;
  this->TheNashGame->isSolved(this->SolutionX, *countryNumber,
                              *profitableDeviation);
  arma::vec objvals =
      this->TheNashGame->computeQPObjectiveValues(this->SolutionX, true);
  for (unsigned int i = 0; i < this->NumPlayers; ++i) {
 ...
 */
{
  ...
  // if (!this->PlayersQP.at(i).get())
  {
    ...
    if (this->Stats.AlgorithmParam.PolyLcp) {
      this->PlayersLCP.at(i)->makeQP(
          *this->LeaderObjectiveConvexHull.at(i).get(),
 ...
 */
{
  ...
  for (unsigned int i = 0; i < this->NumPlayers; ++i) {
    ...
    try {
      ...
      cerr << "GRBException in Game::EPEC::makePlayerQP : " << e.getErrorCode()
           << ": " << e.getMessage() << '\n';
      throw;
    } catch (exception &e) {
...
void Game::EPEC::makeThePureLCP(bool indicators) {
  ...
  try {
        ...
        if (indicators) {
          this->LCPModel->addGenConstrIndicator(
              ...
              this->LCPModel->getVarByName(
                  "x_" + to_string(this->getPositionProbab(i, j))),
    ...
    if (indicators) {
      ...
      BOOST_LOG_TRIVIAL(trace)
          << "Game::EPEC::makeThePureLCP: using indicator constraints.";
    } else {
    ...
    cerr << "GRBException in Game::EPEC::makeThePureLCP : " << e.getErrorCode()
         << ": " << e.getMessage() << '\n';
    throw;
  }
}
...
unsigned int Game::EPEC::getPositionLeadFollPoly(const unsigned int i,
  ...
  if (this->Stats.AlgorithmParam.PolyLcp) {
    const auto LeaderStart = this->TheNashGame->getPrimalLoc(i);
    const auto FollPoly =
        dynamic_cast<Game::PolyLCP *>(this->PlayersLCP.at(i).get())
            ->convPolyPosition(k);
    return LeaderStart + FollPoly + j;
  } else {
    BOOST_LOG_TRIVIAL(warning)
        << "Cannot use Game::EPEC::getPositionLeadFollPoly with the current "
           "Algorithm. Returning 0";
    return 0;
  }
}

unsigned int Game::EPEC::getPositionLeadLeadPoly(const unsigned int i,
                                                 const unsigned int j,
                                                 const unsigned int k) const {
  /**
  ...
  if (this->Stats.AlgorithmParam.PolyLcp) {
    ...
    BOOST_LOG_TRIVIAL(warning)
        << "Cannot use Game::EPEC::getPositionLeadLeadPoly with the current "
           "Algorithm. Returning 0";
    return 0;
  }
}
...
unsigned int Game::EPEC::getNumPolyLead(const unsigned int i) const {
   ...
   */
  if (this->Stats.AlgorithmParam.PolyLcp) {
    return dynamic_cast<Game::PolyLCP *>(this->PlayersLCP.at(i).get())
        ->convNumPoly();
  } else {
    BOOST_LOG_TRIVIAL(warning)
        << "Cannot use Game::EPEC::getNumPolyLead with the current "
           "Algorithm. Returning 0";
    return 0;
  }
}

unsigned int Game::EPEC::getPositionProbab(const unsigned int i,
                                           const unsigned int k) const {
  /**
...
std::vector<unsigned int> Game::EPEC::mixedStrategyPoly(const unsigned int i,
 ...
 */
{
  if (this->Stats.AlgorithmParam.PolyLcp) {
    ...
    BOOST_LOG_TRIVIAL(warning)
        << "Cannot use Game::EPEC::mixedStrategyPoly with the current "
...
double Game::EPEC::getValLeadFoll(const unsigned int i,
  ...
  return this->LCPModel
      ->getVarByName("x_" + to_string(this->getPositionLeadFoll(i, j)))
      .get(GRB_DoubleAttr_X);
}
...
double Game::EPEC::getValLeadLead(const unsigned int i,
  ...
  if (!this->LCPModel)
    throw std::string("Error in Game::EPEC::getValLeadLead: "
                      "Game::EPEC::LCPModel not made and solved");
  return this->LCPModel
      ->getVarByName("x_" + to_string(this->getPositionLeadLead(i, j)))
      .get(GRB_DoubleAttr_X);
}
...
double Game::EPEC::getValLeadFollPoly(const unsigned int i,
  ...
  if (probab > 1 - tol)
    ...
    return this->LCPModel
               ->getVarByName("x_" +
                              to_string(this->getPositionLeadFollPoly(i, j, k)))
               .get(GRB_DoubleAttr_X) /
           probab;
}
...
double Game::EPEC::getValLeadLeadPoly(const unsigned int i,
  ...
  if (probab > 1 - tol)
    ...
    return this->LCPModel
               ->getVarByName("x_" +
                              to_string(this->getPositionLeadLeadPoly(i, j, k)))
               .get(GRB_DoubleAttr_X) /
           probab;
}
