 ...
 */
{
	...
	for (unsigned int i = 0; i != nPoly; i++) {
		if (Ai->at (i)->n_cols != nC)
			throw ("Game::convexHull: Inconsistent number of variables in the "
			       "polyhedra ") + std::to_string (i) + "; " + std::to_string (Ai->at (i)->n_cols) + "!=" +
			      std::to_string (nC);
                 ...
                 */
{
	...
	if (x.size () != this->Nx)
		throw "Game::QP_Param::solveFixed: Invalid argument size: " + std::to_string (x.size ()) + " != " +
		      std::to_string (Nx);
	...
	try {
		...
		std::cerr << "GRBException: Error in Game::QP_Param::solveFixed: " << e.getErrorCode () << "; "
		          << e.getMessage () << '\n';
		throw;
	}
	return model;
...
{
	return this->set (std::move (obj.Q), std::move (obj.C), std::move (cons.A), std::move (cons.B), std::move (obj.c),
	                  std::move (cons.b));
}
...
Game::NashGame::NashGame (const NashGame &N) : Env{N.Env}, LeaderConstraints{N.LeaderConstraints},
...
const Game::NashGame &Game::NashGame::formulateLCP (arma::sp_mat &M, ///< Where the output  M is stored and returned.
	...
	for (unsigned int i = 0; i < NumPlayers; i++) {
		...
		if (i > 0) { // For the first player, no need to add anything 'before' 0-th
			...
			M.submat (this->PrimalPosition.at (i), 0, this->PrimalPosition.at (i + 1) - 1,
			          this->PrimalPosition.at (i) - 1) = Ni[i].submat (0, 0, numPrim - 1,
			                                                           this->PrimalPosition.at (i) - 1);
		}
		// Region 2 in Formulate LCP.ipe
		BOOST_LOG_TRIVIAL(trace) << "Game::NashGame::formulateLCP: Region 2";
		M.submat (this->PrimalPosition.at (i), this->PrimalPosition.at (i), this->PrimalPosition.at (i + 1) - 1,
		...
		if (this->PrimalPosition.at (i + 1) != this->DualPosition.at (0)) {
			M.submat (this->PrimalPosition.at (i), this->PrimalPosition.at (i + 1), this->PrimalPosition.at (i + 1) - 1,
			          this->DualPosition.at (0) - 1) = Ni[i].submat (0, this->PrimalPosition.at (i), numPrim - 1,
			                                                         Ni[i].n_cols - 1);
		}
		...
		if (this->DualPosition.at (i) != this->DualPosition.at (i + 1)) {
			M.submat (this->PrimalPosition.at (i), this->DualPosition.at (i), this->PrimalPosition.at (i + 1) - 1,
			          this->DualPosition.at (i + 1) - 1) = Mi[i].submat (0, numPrim, numPrim - 1,
			                                                             numPrim + numDual - 1);
		}
		// RHS
		BOOST_LOG_TRIVIAL(trace) << "Game::NashGame::formulateLCP: Region RHS";
		q.subvec (this->PrimalPosition.at (i), this->PrimalPosition.at (i + 1) - 1) = qi[i].subvec (0, numPrim - 1);
		...
		if (numDual > 0) {
			if (i > 0) // For the first player, no need to add anything 'before' 0-th
				// position
				M.submat (this->DualPosition.at (i) - numLeaderVar, 0, this->DualPosition.at (i + 1) - numLeaderVar - 1,
				          this->PrimalPosition.at (i) - 1) = Ni[i].submat (numPrim, 0, Ni[i].n_rows - 1,
				                                                           this->PrimalPosition.at (i) - 1);
			...
			M.submat (this->DualPosition.at (i) - numLeaderVar, this->PrimalPosition.at (i),
			          ...
			          this->PrimalPosition.at (i + 1) - 1) = Mi[i].submat (numPrim, 0, numPrim + numDual - 1,
			                                                               numPrim - 1);
			...
			if (this->DualPosition.at (0) != this->PrimalPosition.at (i + 1)) {
				M.submat (this->DualPosition.at (i) - numLeaderVar, this->PrimalPosition.at (i + 1),
				          this->DualPosition.at (i + 1) - numLeaderVar - 1,
				          this->DualPosition.at (0) - 1) = Ni[i].submat (numPrim, this->PrimalPosition.at (i),
				                                                         Ni[i].n_rows - 1, Ni[i].n_cols - 1);
			}
			...
			M.submat (this->DualPosition.at (i) - numLeaderVar, this->DualPosition.at (i),
			          this->DualPosition.at (i + 1) - numLeaderVar - 1,
			          this->DualPosition.at (i + 1) - 1) = Mi[i].submat (numPrim, numPrim, numPrim + numDual - 1,
			                                                             numPrim + numDual - 1);
			// RHS
			BOOST_LOG_TRIVIAL(trace) << "Game::NashGame::formulateLCP: Region RHS";
			q.subvec (this->DualPosition.at (i) - numLeaderVar,
			          this->DualPosition.at (i + 1) - numLeaderVar - 1) = qi[i].subvec (numPrim, qi[i].n_rows - 1);
 ...
 */
{
	...
	try {
		if (A_in.n_rows) {
			...
			A_out_expl.cols (this->LeaderPosition, this->DualPosition.at (0) - 1) = A_in.cols (this->MC_DualPosition,
			                                                                                   n_Col - 1);
		}
		if (this->MCRHS.n_rows) {
			// MC constraints can be written as if they are leader constraints
			A_out_MC.submat (0, 0, this->MCRHS.n_rows - 1, this->DualPosition.at (0) - 1) = this->MarketClearing;
 ...
 */
{
	...
	nEnd = this->PrimalPosition.at (player + 1); // Start of the player+1-th player's primals or LeaderVrs if
...
arma::vec Game::NashGame::computeQPObjectiveValues (const arma::vec &x, bool checkFeas) const {
	...
	for (unsigned int i = 0; i < this->NumPlayers; ++i) {
		...
		if (nEnd < nVar) {
			x_minus_i.subvec (nStart, nVar + nStart - nEnd - 1) = x.subvec (nEnd, nVar -
...
bool Game::NashGame::isSolved (const arma::vec &sol, unsigned int &violPlayer, arma::vec &violSol, double tol) const {
	/**
...
void Game::EPEC::addDummyLead (const unsigned int i ///< The leader to whom dummy variables should be added
) {
...
void Game::EPEC::getXMinusI (const arma::vec &x, const unsigned int &i, arma::vec &solOther) const {
	...
	solOther.zeros (nEPECvars -        // All variables in EPEC
	                ...
	                nThisCountryHullVars);  // We subtract the hull variables associated to the ith player
	// convex hull vars

	for (unsigned int j = 0, count = 0, current = 0; j < this->NumPlayers; ++j) {
		if (i != j) {
			current = *this->LocEnds.at (j) - this->ConvexHullVariables.at (j);
			solOther.subvec (count, count + current - 1) = x.subvec (this->LeaderLocations.at (j),
			                                                         this->LeaderLocations.at (j) + current - 1);
			count += current;
		}
		// We need to keep track of MC_vars also for this country
		solOther.at (solOther.n_rows - this->numMCVariables + j) = x.at (this->NumVariables - this->numMCVariables + j);
	}
}

void Game::EPEC::getXofI (const arma::vec &x, const unsigned int &i, arma::vec &solI) const {
	/*
 * Given the player id @p i and the solution @p x, the method returns in @p xWithoutHull the x vector for the given player,
 * with the convex-hull's variables.
 */
	const unsigned int nThisCountryvars = *this->LocEnds.at (i);
	const unsigned int nThisCountryHullVars = this->ConvexHullVariables.at (i);
	...
	solI.subvec (0, vars - 1) = x.subvec (this->LeaderLocations.at (i), this->LeaderLocations.at (i) + current - 1);
}

void Game::EPEC::getXWithoutHull (const arma::vec &x, const unsigned int &i, arma::vec &xWithoutHull) const {
	/*
	 * Given the player id @p i and the solution @p x, the method returns in @p xWithoutHull the x vector for the given player,
	 * without the convex-hull's variables.
	 */
	const unsigned int nEPECvars = this->NumVariables;
	const unsigned int nConvexHullVars = static_cast<const unsigned int>(
			std::accumulate (this->ConvexHullVariables.rbegin (), this->ConvexHullVariables.rend (), 0));
	...
	xWithoutHull.zeros (nEPECvars -        // All variables in EPEC
	                    nConvexHullVars);  // We subtract the hull variables associated to the convex hull
	// convex hull vars

	for (unsigned int j = 0, count = 0, current = 0; j < this->NumPlayers; ++j) {
		current = *this->LocEnds.at (j) - this->ConvexHullVariables.at (j);
		xWithoutHull.subvec (count, count + current - 1) = x.subvec (this->LeaderLocations.at (j),
		                                                             this->LeaderLocations.at (j) + current - 1);
		count += current;
		// We need to keep track of MC_vars also for this country
		xWithoutHull.at (xWithoutHull.n_rows - this->numMCVariables + j) = x.at (
				this->NumVariables - this->numMCVariables + j);
	}
}
...
std::unique_ptr<GRBModel> Game::EPEC::respond (const unsigned int i, const arma::vec &x) const {
	...
	return this->PlayersLCP.at (i).get ()->MPECasMILP (this->LeaderObjective.at (i).get ()->C,
...
bool Game::EPEC::isSolved (unsigned int *countryNumber, arma::vec *profitableDeviation, double tol) const
/**
 ...
 */
{
	if (!this->TheNashGame)
		return false;
	if (!this->NashEquilibrium)
		return false;
	if (tol < 0)
		tol = this->Stats.AlgorithmParam.DeviationTolerance;
	this->TheNashGame->isSolved (this->SolutionX, *countryNumber, *profitableDeviation, tol);
	arma::vec objvals = this->TheNashGame->computeQPObjectiveValues (this->SolutionX, true);
	for (unsigned int i = 0; i < this->NumPlayers; ++i) {
		double val = this->respondSol (*profitableDeviation, i, this->SolutionX);
		if (val == GRB_INFINITY)
			return false;
		if (std::abs (val - objvals.at (i)) > tol) {
			*countryNumber = i;
			BOOST_LOG_TRIVIAL(trace) << "Game::EPEC::isSolved: deviation for player " << i << " -- of "
 ...
 */
{
	...
	for (unsigned int i = 0; i < this->NumPlayers; ++i) {
		...
		unsigned int convHullVarCount = this->LeaderObjectiveConvexHull.at (i)->Q.n_rows - originalSizeWithoutHull;
		...
		try {
			if (this->NumPlayers > 1) {
				for (unsigned int j = 0; j < this->NumPlayers; j++) {
					if (i != j) {
						this->PlayersQP.at (j)->addDummy (convHullVarCount, 0, this->PlayersQP.at (j)->getNx () -
...
void ::Game::EPEC::makeTheLCP () {
	...
	BOOST_LOG_TRIVIAL(trace) << "Game::EPEC::makeTheLCP(): Indicators set to " << this->Stats.AlgorithmParam.Indicators;
	this->TheLCP->UseIndicators = this->Stats.AlgorithmParam.Indicators; // Using indicator constraints
...
bool Game::EPEC::computeNashEq (bool pureNE,           ///< True if we search for a PNE
	...
	try { // Try finding a Nash equilibrium for the approximation
		...
		BOOST_LOG_TRIVIAL(error) << "GRBException in Game::EPEC::computeNashEq : " << e.getErrorCode () << ": "
		                         << e.getMessage () << " ";
	}
	if (this->NashEquilibrium) { // If a Nash equilibrium is found, then update
		...
		if (check) {
			...
			for (int k = 0, stop = 0; k < scount && stop == 0; ++k) {
				...
				this->NashEquilibrium = this->TheLCP->extractSols (this->LCPModel.get (), this->SolutionZ,
				                                                   this->SolutionX, true);
				if (this->isSolved ()) {
...
bool Game::EPEC::warmstart (const arma::vec x) {
	...
	if (x.size () < this->getNumVar ()) {
		BOOST_LOG_TRIVIAL(error) << "Exception in Game::EPEC::warmstart: number of variables "
		                            "does not fit this instance.";
		throw;
	}
	if (!this->Finalized) {
		BOOST_LOG_TRIVIAL(error) << "Exception in Game::EPEC::warmstart: EPEC is not Finalized.";
		throw;
	}
	if (this->PlayersQP.front () == nullptr) {
...
void Game::EPEC::makeThePureLCP (bool indicators) {
	/**
	...
	try {
		...
		this->LCPModelBase = std::unique_ptr<GRBModel> (new GRBModel (*LCPModel));
...
const void Game::EPEC::findNashEq () {
	...
	switch (this->Stats.Status) {
		...
		case Game::EPECsolveStatus::NashEqFound: {
			final_msg << "Found a Nash equilibrium (" << (this->Stats.PureNashEquilibrium == 0 ? "MNE" : "PNE") << ").";
		}
			break;
...
unsigned int
Game::EPEC::getPositionLeadFollPoly (const unsigned int i, const unsigned int j, const unsigned int k) const {
	/**
	 * Get the position of the k-th follower variable of the i-th leader, in the
	 * j-th feasible polyhedron.
	 *
	 * Indeed it should hold that @f$ j < @f$ Game::EPEC::getNumPolyLead(i)
	 */
	if (this->Stats.AlgorithmParam.PolyLcp) {
		const auto LeaderStart = this->TheNashGame->getPrimalLoc (i);
		const auto FollPoly = dynamic_cast<Game::PolyLCP *>(this->PlayersLCP.at (i).get ())->convPolyPosition (k);
		return LeaderStart + FollPoly + j;
	} else {
		BOOST_LOG_TRIVIAL(warning) << "Cannot use Game::EPEC::getPositionLeadFollPoly with the current "
...
unsigned int
Game::EPEC::getPositionLeadLeadPoly (const unsigned int i, const unsigned int j, const unsigned int k) const {
	/**
	...
	if (this->Stats.AlgorithmParam.PolyLcp) {
		const auto LeaderStart = this->TheNashGame->getPrimalLoc (i);
		const auto FollPoly = dynamic_cast<Game::PolyLCP *>(this->PlayersLCP.at (i).get ())->convPolyPosition (k);
		return LeaderStart + FollPoly + this->PlayersLCP.at (i)->getLStart () + j;
	} else {
		BOOST_LOG_TRIVIAL(warning) << "Cannot use Game::EPEC::getPositionLeadLeadPoly with the current "
		                              "Algorithm. Returning 0";
		return 0;
	}
}

unsigned int Game::EPEC::getNumPolyLead (const unsigned int i) const {
	/**
	 * Get the number of polyhedra used in the inner approximation of the
	 * feasible region of the i-th leader.*
	 */
	if (this->Stats.AlgorithmParam.PolyLcp) {
		return dynamic_cast<Game::PolyLCP *>(this->PlayersLCP.at (i).get ())->convNumPoly ();
	} else {
		BOOST_LOG_TRIVIAL(warning) << "Cannot use Game::EPEC::getNumPolyLead with the current "
		                              "Algorithm. Returning 0";
		return 0;
	}
}

unsigned int Game::EPEC::getPositionProbab (const unsigned int i, const unsigned int k) const {
	/**
	 ...
	 */
	if (this->Stats.AlgorithmParam.PolyLcp) {
		const auto PolyProbab = dynamic_cast<Game::PolyLCP *>(this->PlayersLCP.at (i).get ())->convPolyWeight (k);
		if (PolyProbab == 0)
			return 0;
		const auto LeaderStart = this->TheNashGame->getPrimalLoc (i);
...
bool Game::EPEC::isPureStrategy (const unsigned int i, const double tol) const {
	...
	if (this->Stats.AlgorithmParam.PolyLcp) {
		...
		BOOST_LOG_TRIVIAL(warning) << "Cannot use Game::EPEC::isPureStrategy with the current "
		                              "Algorithm. Returning 0";
		return false;
	}
}

std::vector<unsigned int> Game::EPEC::mixedStrategyPoly (const unsigned int i, const double tol) const
/**
...
double Game::EPEC::getValProbab (const unsigned int i, const unsigned int k) const {
	if (this->Stats.AlgorithmParam.PolyLcp) {
		const unsigned int varname{this->getPositionProbab (i, k)};
		if (varname == 0)
			return 1;
		return this->LCPModel->getVarByName ("x_" + std::to_string (varname)).get (GRB_DoubleAttr_X);
	} else {
		BOOST_LOG_TRIVIAL(warning) << "Cannot use Game::EPEC::mixedStrategyPoly with the current "
		                              "Algorithm. Returning 0";
		return 0;
...
double Game::EPEC::getValLeadFoll (const unsigned int i, const unsigned int j) const {
	...
	return this->LCPModel->getVarByName ("x_" + std::to_string (this->getPositionLeadFoll (i, j))).get (
			GRB_DoubleAttr_X);
}
...
double Game::EPEC::getValLeadLead (const unsigned int i, const unsigned int j) const {
	if (!this->LCPModel)
		throw std::string ("Error in Game::EPEC::getValLeadLead: "
		                   "Game::EPEC::LCPModel not made and solved");
	return this->LCPModel->getVarByName ("x_" + std::to_string (this->getPositionLeadLead (i, j))).get (
			GRB_DoubleAttr_X);
}
...
double Game::EPEC::getValLeadLeadPoly (const unsigned int i, const unsigned int j, const unsigned int k,
                                       const double tol) const {
	if (!this->LCPModel)
