...
Models::FollPar operator+(const Models::FollPar &F1,
  ...
  cq.insert(cq.back(), F1.costs_quad.begin(), F1.costs_quad.end());
  cq.insert(cq.back(), F2.costs_quad.begin(), F2.costs_quad.end());

  cl.insert(cl.back(), F1.costs_lin.begin(), F1.costs_lin.end());
  cl.insert(cl.back(), F2.costs_lin.begin(), F2.costs_lin.end());

  cap.insert(cap.back(), F1.capacities.begin(), F1.capacities.end());
  cap.insert(cap.back(), F2.capacities.begin(), F2.capacities.end());

  ec.insert(ec.back(), F1.emission_costs.begin(), F1.emission_costs.end());
  ec.insert(ec.back(), F2.emission_costs.begin(), F2.emission_costs.end());

  tc.insert(tc.back(), F1.tax_caps.begin(), F1.tax_caps.end());
  tc.insert(tc.back(), F2.tax_caps.begin(), F2.tax_caps.end());

  nm.insert(nm.back(), F1.names.begin(), F1.names.end());
  nm.insert(nm.back(), F2.names.begin(), F2.names.end());
 ...
 */
{
  if (!chkAllLeadPars && AllLeadPars.size() != this->nCountr)
    return false;
  if (!chkcountries_LL && countries_LL.size() != this->nCountr)
    return false;
  if (!chkMC_QP && MC_QP.size() != this->nCountr)
    return false;
  if (!chkLeadConses && LeadConses.size() != this->nCountr)
    return false;
  if (!chkLeadRHSes && LeadRHSes.size() != this->nCountr)
    return false;
  if (!chknImportMarkets && nImportMarkets.size() != this->nCountr)
    return false;
  if (!chkLocations && Locations.size() != this->nCountr)
    return false;
...
void Models::EPEC::writeSolutionJSON(string filename, const arma::vec x,
  ...
  for (unsigned i = 0; i < this->nCountr; i++) {
    ...
    writer.Uint(this->getPosition(this->nCountr - 1, Models::LeaderVars::End) +
                i);
    writer.EndObject();
  }
  writer.EndArray();
...
void Models::EPEC::WriteCountry(const unsigned int i, const string filename,
  ...
  double exportPrice{
      x.at(this->getPosition(this->nCountr - 1, Models::LeaderVars::End) + i)};
  double import{0};
