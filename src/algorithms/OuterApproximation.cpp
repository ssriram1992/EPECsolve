...
void Algorithms::OuterApproximation::solve () {
	...
	while (!solved) {
		...
		while (p < this->EPECObject->NumPlayers) {
			...
			if (incumbent.at (p)->getCumulativeBranches () == Trees.at (p)->getEncodingSize ())
				comp++;
			else {
				...
				if (branchingLocations.at (0) > -1 && branchingLocations.at (1) > -1 &&
					...
					branches = Trees.at (p)->multipleBranch (branchingLocations, *incumbent.at (p));
					BOOST_LOG_TRIVIAL(info) << "Algorithms::OuterApproximation::solve: Multiple branching on "
					                        << branchingLocations.at (0) << " and " << branchingLocations.at (1);
				} else {
					if (branchingLocations.at (0) > -1) {
						BOOST_LOG_TRIVIAL(info) << "Algorithms::OuterApproximation::solve: Player " << p
						                        << " - branching (Infeasible)  on " << branchingLocations.at (0);
						branches = Trees.at (p)->singleBranch (branchingLocations.at (0), *incumbent.at (p));
						...
						branches = Trees.at (p)->singleBranch (branchingLocations.at (1), *incumbent.at (p));
						branchingLocations = {branchingLocations.at (1)};
					} else if (branchingLocations.at (0) < 0 && branchingLocations.at (1) < 0) {
						BOOST_LOG_TRIVIAL(info) << "Algorithms::OuterApproximation::solve: Player " << p
						                        << " - branching (FirstAvailable)  on " << branchingLocations.at (2);
						branches = Trees.at (p)->singleBranch (branchingLocations.at (2), *incumbent.at (p));
				...
				if (branches.at (0) < 0) {
					BOOST_LOG_TRIVIAL(error) << "Algorithms::OuterApproximation::solve: Player " << p
					                         << " - branching  on -- cannot branch.";
					throw;
				}
				//Check if any of the two children is infeasible
				auto childEncoding = this->Trees.at (p)->getNodes ()->at (branches.at (0)).getEncoding ();
				this->outerLCP.at (p)->outerApproximate (childEncoding, true);
				if (!this->outerLCP.at (p)->getFeasApprox ()) {
					//This child is infeasible
					Trees.at (p)->denyBranchingLocations (*incumbent.at (p), branchingLocations);
					p--;
				} else {
					incumbent.at (p) = &(this->Trees.at (p)->getNodes ()->at (branches.at (0)));
				}
		...
		if (comp == this->EPECObject->NumPlayers) {
			BOOST_LOG_TRIVIAL(info) << "Algorithms::OuterApproximation::solve: Solved without any equilibrium.";
			this->EPECObject->Stats.Status = Game::EPECsolveStatus::NashEqNotFound;
			solved = true;
			break;
		}
		...
		this->EPECObject->computeNashEq (this->EPECObject->Stats.AlgorithmParam.PureNashEquilibrium);
		if (this->EPECObject->isSolved ()) {
			this->EPECObject->Stats.Status = Game::EPECsolveStatus::NashEqFound;
			BOOST_LOG_TRIVIAL(info) << "Algorithms::OuterApproximation::solve: Solved";
			break;
		}
...
int Algorithms::OuterApproximation::getFirstBranchLocation (const unsigned int player, const OuterTree::Node *node) {
	/**
 * Given @p player -- containing the id of the player, returns the branching decision for that node, with no complementarity condition enforced.
	 * In particular, the method return the (positive) id of the complementarity equation if there is a feasible branching decision at @p node,
	 * and a negative value otherwise.
	 * @return a positive int with the id of the complementarity to branch on, or a negative value if none exists.
 */
...
std::vector<int>
Algorithms::OuterApproximation::getNextBranchLocation (const unsigned int player, const OuterTree::Node *node) {
	/**
 * Given @p player -- containing the id of the player -- and @p node containing a node, returns the branching decision for that node, with respect to the current node.
	 * In particular, the method return the (positive) id of the complementarity equation if there is a feasible branching decision at @p node,
	 * and a negative value otherwise.
	 * @return a positive int with the id of the complementarity to branch on, or a negative value if none exists.
 */
	...
	if (this->EPECObject->NashEquilibrium) {
		//There exists a Nash Equilibrium for the outer approximation, which is not a Nash Equilibrium for the game
		arma::vec x, z;
		...
		for (unsigned int i = 0; i < currentSolution.size (); i++) {
			unsigned int varPos =
					i >= this->outerLCP.at (player)->getLStart () ? i + this->outerLCP.at (player)->getNumberLeader ()
					                                              : i;
			if (x (varPos) > 0 && z (i) > 0 && node->getAllowedBranchings ().at (i) && currentSolution.at (i) == 0) {
	...
	if (decisions.at (0) < 0 && decisions.at (1) < 0) {
		...
		BOOST_LOG_TRIVIAL(info) << "Player " << player << ": branching with DeviationBranching is feasible";
	} else if (decisions.at (0) >= 0) {
		BOOST_LOG_TRIVIAL(info) << "Player " << player << ": branching with MostInfeasibleBranching is feasible";
...
void Algorithms::OuterApproximation::printCurrentApprox () {
	/*
	 * Returns a log message containing the encoding at the current outer approximation iteration
	 */
...
OuterTree::Node::Node (Node &parent, unsigned int idComp, unsigned long int id) {
	/*
 * Given the parent node address @param parent, the @param idComp to branch on, and the @param id, creates a new node
 */
...
void OuterTree::denyBranchingLocation (OuterTree::Node &node, const unsigned int &location) {
	/*
* If a complementarity equation @param location  has proven to be infeasible or it isn't a candidate for branching, this method prevents any further branching on it for the node @param node.
*/
	...
	if (!node.AllowedBranchings.at (location))
		BOOST_LOG_TRIVIAL(warning) << "OuterTree::denyBranchingLocation: location has been already denied.";
	node.AllowedBranchings.at (location) = false;
}

void OuterTree::denyBranchingLocations (OuterTree::Node &node, const std::vector<int> &locations) {
	/*
* If a complementarity equation @param location  has proven to be infeasible or it isn't a candidate for branching, this method prevents any further branching on it for the node @param node.
*/
...
std::vector<long int> OuterTree::singleBranch (const unsigned int idComp, OuterTree::Node &t) {
	/*
	 * Given the @param idComp and the parent node @param t, creates a single child by branching on @param idComp.
	 */
	if (idComp >= this->EncodingSize)
		throw "OuterTree::branch idComp is larger than the encoding size.";
	if (t.Encoding.at (idComp) != 0) {
		BOOST_LOG_TRIVIAL(warning) << "OuterTree: cannot branch on this complementary, since it already has been processed.";
...
std::vector<long int> OuterTree::multipleBranch (const std::vector<int> idsComp, Node &t) {
	/*
 * Given the @param idComp and the parent node @param t, creates a single child by branching on @param idComp.
 */
...
OuterTree::Node::Node (Node &parent, std::vector<int> idsComp, unsigned long int id) {
	/*
 * Given the parent node address @param parent, the @param idsComp to branch on (containing all the complementarities ids), and the @param id, creates a new node
 */
