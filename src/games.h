 ...
 * @code
	...
	C(0, 1) = 2;
	C(2, 0) = 3;
	vec c(3);						// The vector for linear terms in y
 ...
 * @f}
 *
 * But this computation need not be done manually by supplying the value of @f$x@f$ and solving using Gurobi.
 * @code
 ...
 * @endcode
 * @p FixedModel has the GRBModel object, and all operations native to GRBModel, like accessing the value of a variable, a dual
 *	multiplier, saving the problem to a .lp file or a .mps file etc. can be performed on the object. In particular, the solution can be compared with hand-calculated solution as shown below.
 *	@code
        arma::vec sol(3);
 ...
 *	@endcode
 *
 * In many cases, one might want to obtain the KKT conditions of a convex quadratic program and that can be obtained as below, using @p QP_Param::KKT. 
 *
 * The function returns @p M, @p N and @p q, where the KKT conditions can be written as 
 * @f[
 ...
 *
 * Before reading this page, please ensure you are aware of the functionalities described in @link QP_Param_Example Game::QP_Param tutorial @endlink before following this page.
 *
 * @b PLAYER @b 1:
 * @f[
 * 	\min_{q_1}: 10 q_1 + 0.1 q_1^2 - (100 - (q_1+q_2)) q_1 	= 1.1 q_1^2 - 90 q_1 + q_1q_2
 * 	@f]
 * 	 s.t:
 * 	 @f[
 * 	 	q_1 >= 0
 ...
 * 	 	@f]
 *
 * The above problem corresponds to a <a href="https://en.wikipedia.org/wiki/Cournot_competition">Cournot Competition</a> where the demand curve is given by @f$ P = a-BQ @f$ where @p P is the market price and @p Q is the quantity in the market. The cost of production of both the producers are given by a convex quadratic function in the quantity they produce. The solution to the problem is to find a <a href="https://en.wikipedia.org/wiki/Nash_equilibrium"> Nash equilibrium </a> from which neither producer is incentivized to deviate.
 *
 * To handle this problem, first we create two objects of Game::QP_Param to model each player's optimization problem, as parameterized by the other.
 * @code
 ...
 * @f}
 *
 * To observe the LCP formulation of this NashGame, one can use Game::NashGame::FormulateLCP member function.
 * @code
 * 	arma::sp_mat M;
 * 	arma::vec q;
 ...
 * @endcode
 * 
 * Here @p M and @p q are such that the solution to the LCP @f$ 0 \le x \perp Mx + q \ge 0 @f$ solves the original NashGame. These matrices can be written to a file and solved externally now.
 *
 * Alternatively, one can pass it to the Game::LCP class, and solve it natively. To achieve this, one can pass the above matrices to the constructor of the Game::LCP class.
 * @code
 	GRBEnv env = GRBEnv();
	Game::LCP lcp = Game::LCP(&env, M, q, 1, 0);
 * @endcode
 *
 * More concisely, the class Game::LCP offers a constructor with the NashGame itself as an argument. This way, one need not explicitly compute @p M, @p q etc., to create the Game::LCP object.
 * @code
 ...
 * @code
	...
	model.optimize();			// Alternatively, auto model = lcp.LCPasMIP(true); will already optimize and solve the model.
 * @endcode
